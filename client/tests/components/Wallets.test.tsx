...
describe('Wallets Component', () => {
  ...
  it('renders wallets correctly', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Your Wallets')).toBeTruthy());

    mockWallets.forEach((wallet) => {
      expect(screen.getByText(wallet.name)).toBeTruthy();
      expect(
        screen.getByText(wallet.address.slice(0, 30) + '...')
      ).toBeTruthy();
    });
  });
  ...
  it('removes wallet on delete', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Wallet 1')).toBeTruthy());
