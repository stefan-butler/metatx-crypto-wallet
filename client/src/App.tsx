...
//we dont actually need that i guess. u need to install chrome package for it to work without that
declare global {
  interface Window {
...
function App() {
  ...
  const importWalletFromMnemonic = (mnemonicInput: string) => {
    ...
    try {
      ...
      createWallet({address: walletFromMnemonic.address, mnemonic: walletFromMnemonic.mnemonic.phrase, privateKey: walletFromMnemonic.privateKey, name: walName});
      setWalName("");
      setMnemonic("");
  ...
  const importWalletFromPrivateKey = (privateKeyInput: string): void => {
    ...
    try {
      ...
      createWallet({address: importedWallet.address, mnemonic: "pk imported", privateKey: importedWallet.privateKey, name: walName});
      setWalName("");
      setPrivateKey("");
